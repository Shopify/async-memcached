...
#[tokio::test]
#[parallel]
async fn test_raw_pipe() {
    let key = "raw-pipe-meta-get-key-missing";
    let meta_get_command = format!("mg #{key} v f\r\n");

    let mut client = setup_client(&[key]).await;

    let result = client.raw_pipe(meta_get_command.as_bytes()).await;
    assert!(result.is_ok());
}

#[tokio::test]
async fn test_raw_pipe_meta_get_key_exists() {
    let key = "raw-pipe-meta-get-key-exists";
    let value = "value";
    ...
    // set the key
    let set_command = format!("ms {key} {value_len} v f\r\n{value}\r\n");
    let result = client.raw_pipe(set_command.as_bytes()).await;
    assert!(result.is_ok());

    let result = client.raw_pipe(meta_get_command.as_bytes()).await;
    assert!(result.is_ok());
    assert_eq!(result.unwrap(), b"VA 5 f0\r\nvalue\r\n");
}

#[tokio::test]
async fn test_raw_pipe_meta_set() {
    let key = "raw-pipe-meta-set";
    ...
    let result = client.raw_pipe(meta_set_command.as_bytes()).await;
    assert!(result.is_ok());
    assert_eq!(result.unwrap(), b"HD\r\n");
}

#[tokio::test]
async fn test_raw_pipe_meta_set_large_value() {
    let key = "dump-raw-bytes-meta-set-key-missing";
    let v = "a".repeat(LARGE_PAYLOAD_SIZE);
    let value_len = v.len();
    let meta_set_command = format!("ms {key} {value_len} v f\r\n{v}\r\n");
