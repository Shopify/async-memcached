...
fn parse_meta_error(buf: &[u8]) -> IResult<&[u8], Response> {
...
fn parse_meta_incrdecr(buf: &[u8]) -> IResult<&[u8], Response> {
...
fn parse_meta_value(buf: &[u8]) -> IResult<&[u8], Value> {
...
fn parse_meta_data(buf: &[u8]) -> IResult<&[u8], Response> {
    let values = map(
        fold_many0(
            parse_meta_value,
            || None,
...
mod tests {
    ...
    fn test_regular_complete_parsing() {
        ...
        for (data, data_read, expected) in VALID_NORMAL_CASES.iter() {
            let (n, result) = parse_meta_response(data).unwrap().unwrap();
    ...
    fn test_regular_incomplete_parsing() {
        ...
        for (data, data_read, expected) in VALID_NORMAL_CASES.iter() {
            ...
            while i < *data_read {
                let subbuf = &data[..i];
                assert_eq!(parse_meta_response(subbuf), Ok(None));
                i += 1;
            }
