...
mod meta_parser;
pub use meta_parser::{
    parse_meta_delete_response, parse_meta_get_response, parse_meta_set_response,
};
...
pub enum MetaResponse {
    /// The status of a given operation, which may or may not have succeeded.
    Status(Status),
    /// Data response, which is only returned for when requested via the 'v' meta flag.
