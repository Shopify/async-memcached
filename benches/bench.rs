...
fn bench_set_multi_no_clone_large_strings(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();

    c.bench_function("set_multi_no_clone_large_strings", |b| {
        b.to_async(&rt).iter_custom(|iters| async move {
            let mut client = setup_client().await;
            let large_payload = "a".repeat(LARGE_PAYLOAD_SIZE);
            let kv = vec![
...
fn bench_set_multi_loop_large_strings(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();

    c.bench_function("set_multi_loop_large_strings", |b| {
        b.to_async(&rt).iter_custom(|iters| async move {
            let mut client = setup_client().await;
            let large_payload = "a".repeat(LARGE_PAYLOAD_SIZE);
            let kv = vec![
...
fn bench_set_multi_test_one_large_strings(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();

    c.bench_function("set_multi_test_one_large_strings", |b| {
        b.to_async(&rt).iter_custom(|iters| async move {
            let mut client = setup_client().await;
            let large_payload = "a".repeat(LARGE_PAYLOAD_SIZE);
            let kv = vec![
            ...
            let start = std::time::Instant::now();
            for _ in 0..iters {
                let _ = client.set_multi_test_one(kv.clone(), None, None).await;
            }
            start.elapsed()
        });
    });
...
fn bench_set_multi_test_two_large_strings(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();

    c.bench_function("set_multi_test_two_large_strings", |b| {
        b.to_async(&rt).iter_custom(|iters| async move {
            let mut client = setup_client().await;
            let large_payload = "a".repeat(LARGE_PAYLOAD_SIZE);
            let kv = vec![
...
fn bench_set_multi_test_one_with_100_large_string_values(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();

    c.bench_function("set_multi_test_one_with_100_large_string_values", |b| {
        b.to_async(&rt).iter_custom(|iters| async move {
            let mut client = setup_client().await;
...
fn bench_set_multi_loop_with_100_large_string_values(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();

    c.bench_function("set_multi_loop_with_100_large_string_values", |b| {
        b.to_async(&rt).iter_custom(|iters| async move {
            let mut client = setup_client().await;
